...
# Fucntion to compile images into the executable using Pyinstaller
...
def reset_game():
    ...
    newImg = ImageTk.PhotoImage(Image.open(resource_path("cover.jpg")).resize((150,150)))
...
def play_music():
    ...
    if not music_playing:
        mixer.music.load(resource_path('mystery.mp3')) #Loading Music File
...
def get_sound(url):
    r = requests.get(url)
    with open(resource_path('mystery.mp3'), 'wb') as f:
        f.write(r.content)
        f.close()
...
def get_img(url):
    r = requests.get(url)
    with open(resource_path('cover.jpg'), 'wb') as f:
        f.write(r.content)
        f.close()
...
root.iconbitmap(resource_path('M.ico'))    # Set icon for window
...
play_img = ImageTk.PhotoImage(Image.open(resource_path("p2.png")).resize((30,30)))
...
coverImg = ImageTk.PhotoImage(Image.open(resource_path("cover.jpg")).resize((150,150)))
